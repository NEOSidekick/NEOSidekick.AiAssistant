...
class ConfigurationModuleSigningKeyTest extends FunctionalTestCase
{
     ...
     * GET through as a safe method - so the verb itself is the gate: a GET is refused and rotates
     * nothing, whoever crafted the link.
     *
     * Neos 9's module controller only passes a redirect of a module response on; the 405 the
     * action answers with is rendered into the module page instead.
     */
    #[Test]
    public function aGetOnTheRegenerateActionIsRefusedAndRotatesNothing(): void
    {
        ...
        self::assertSame('', $response->getHeaderLine('Location'), 'no redirect back to a rotated module');
        self::assertStringContainsString('The signing key is only regenerated on a POST from the module form.', (string)$response->getBody());
        self::assertCount(0, $this->requestHistory, 'nothing was pushed');
    ...
    public function anAdministratorRegeneratesTheKeyThroughTheModule(): void
    {
        ...
        self::assertStringContainsString('ai-assistant/configuration', $response->getHeaderLine('Location'), 'back to the module index');
    ...
    public function aRejectedRegenerationKeepsBothPairsAndASecondPressTransmitsTheSamePendingKey(): void
    {
        ...
        $response = $this->postRegenerate();
        self::assertStringContainsString('ai-assistant/configuration', $response->getHeaderLine('Location'), 'a failure redirects back to the module too');
    ...
    private function getModuleIndex(): string
    {
        ...
        $response = $this->objectManager->get(Dispatcher::class)->dispatch($actionRequest);
     ...
     */
    private function postRegenerate(array $moduleArguments = []): ResponseInterface
    {
        $arguments = [
        ...
        return $this->objectManager->get(Dispatcher::class)->dispatch($actionRequest);
    }
     ...
     */
    private function getRegenerate(): ResponseInterface
    {
